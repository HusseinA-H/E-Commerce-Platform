...
const performanceKeywords = ['compression', 'training', 'sprint', 'flex', 'aero', 'titan', 'storm', 'power', 'velocity', 'motion', 'core', 'performance'];
...
export default function PerformancePage() {
  ...
  const products = useMemo(() => {
    return allProducts.filter((p) =>
      ...
      performanceKeywords.some((kw) => p.name.toLowerCase().includes(kw))
    );
  }, [allProducts]);
    ...
    <div className="bg-background text-on-background min-h-screen">
      <header className="px-4 md:px-margin-desktop py-xl max-w-container-max mx-auto">
        <nav className="flex items-center gap-xs font-label-caps text-label-caps text-on-surface-variant/40 mb-md text-[10px] tracking-widest">
          ...
          <span className="text-tertiary">{t('nav.performance')}</span>
        </nav>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-md border-b border-white/5 pb-lg">
          <div>
            <h1 className="font-display-lg text-balance text-3xl md:text-5xl uppercase tracking-tight text-white">{t('nav.performance')}</h1>
            <p className="text-on-surface-variant mt-sm max-w-lg text-sm leading-relaxed text-pretty">
              {t('shop.performanceDesc')}
            </p>
          </div>
        ...
        <section className="flex-grow">
          {isPending ? (
            ...
            <div className="py-xxl text-center space-y-md">
              ...
              <h3 className="font-headline-lg text-balance text-lg text-white">{t('shop.noPerformance')}</h3>
              <p className="text-on-surface-variant text-sm text-pretty leading-relaxed">{t('shop.noPerformanceDesc')}</p>
              <Link
                href="/shop"
